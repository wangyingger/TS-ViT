...
class OxfordFlowers(Dataset):
    def __init__(self, root, train=True, transform=None):
        ...
        # Output the minimum and maximum values of the labels
        print("Minimum label value:", np.min(self.labels))
        print("Maximum label value:", np.max(self.labels))

        # Load set IDs
        setid_path = os.path.join(self.root + '/setid.mat')
        set_ids = scipy.io.loadmat(setid_path)

        # Combine train and validation sets for training
        if self.train:
            train_ids = torch.from_numpy(set_ids['trnid'][0].astype(np.int64))
            valid_ids = torch.from_numpy(set_ids['valid'][0].astype(np.int64))
            self.set_ids = torch.cat((train_ids, valid_ids), dim=0)

        # Use test set for validation/testing
        else:
            self.set_ids = torch.from_numpy(set_ids['tstid'][0].astype(np.int64))

    def __getitem__(self, idx):
        image_id = int(self.set_ids[idx])
        image_path = f'images/image_{image_id:05d}.jpg'
        target = int(self.labels[image_id - 1]) - 1  # Adjusting label to start from 0
